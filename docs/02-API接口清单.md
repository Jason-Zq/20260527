...
- **69.** `GET /api/admin/request-logs` — API 请求记录。Query：`source`/`method`/`path`/`since`/`until`/`limit`/`offset`
- **70.** `GET /api/admin/external-api-logs` — 外部接口调用记录（refresh_url/llm）。Query：`service`/`status`/`batch_id`/…
