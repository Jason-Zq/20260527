...
- **生产 8.138.111.12 有 ~470 个 Office 文件(doc/docx/xls/xlsx)卡 `pending`**(2026-07-28 诊断):不是识别不了,是 stale 任务残留(该机器部署早于 resume-stale 功能);部署新版后点「恢复中断任务」或对家庭「重新生成画像」即可消化,嵌图 OCR 生效后原落 other 的扫描贴图文档会正常分类提取。该机 pip 依赖(xlrd/olefile/python-docx/openpyxl)与 soffice(/usr/local/bin/soffice)均已确认就位;IOD 机部署时同样要确认 **LibreOffice 已装**(Office 预览 preview-pdf 与 .doc 兜底依赖,原为可选现为准必需)。
