...
- **提示词库**(2026-08-06):前端 `/archive-prompts`(ArchivePromptsPage.vue,菜单「文件留底」组)→ `/api/archive-detect/admin/prompts` CRUD + `POST .../generate-prompt2-preview`(编辑弹窗内「AI 重新生成」提示词2,只生成不入库,2026-08-10 由行内 regenerate-prompt2 改来)+ `POST .../{id}/apply-to-overall1`(2026-08-10,总体1 走库开关)+ `GET .../prompts/default-template`(提示词1 重置用);五元组冲突 409。
