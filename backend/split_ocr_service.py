...
与 ocr_service.py 解耦,只服务于"处理超长PDF文件"流水线:
  ...
  - 降 DPI 到 200(证件类字号 16-24pt,200dpi 足够 PaddleOCR 识别)
  - **单线程串行 OCR**,复用 ocr_service 全局单例,避免每线程一份 ~600MB 模型权重把内存打爆
...
def _ocr_one(img_path: str) -> tuple[str, list]:
    ...
    通过 ocr_service.run_ocr 复用全局单例并串行化,跨线程池调用安全。
    """
...
def split_extract_all_pages(
    ...
    """拆分专用:全页 OCR,降 DPI 渲染 + 单线程串行推理。

    Args:
        pdf_path: 源 PDF 路径
        task_id: 任务 ID,渲染后的 PNG 存到 output/{task_id}/images/
        dpi: PDF 渲染 DPI,默认 150(证件类字号 16-24pt,150dpi 足够分类识别,渲染更快)
        max_workers: 兼容旧接口保留参数,实际固定按 1 处理(复用全局 PaddleOCR 单例,
                     多线程共享 PaddleOCR 2.x 实例不稳;改单线程后内存峰值可控)
    ...
    for i in range(total_pages):
        ...
        pil_image, _shrunk = ocr_service._downscale_if_too_large(pil_image)
    ...
    # Step 2: 串行 OCR(全局单例 PaddleOCR 不允许并发)
    results: list[dict] = []
    for page_no, abs_path, filename in img_paths:
        try:
            text, details = _ocr_one(abs_path)
    ...
    print(f"  [split_ocr] OCR 完成: {total_pages} 页(单线程,复用全局 PaddleOCR)")
    return results
